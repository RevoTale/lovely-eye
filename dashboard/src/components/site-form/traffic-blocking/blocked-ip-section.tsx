...
import { normalizeIPInput } from '@/components/site-form/utils';
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          <Input
            ...
            disabled={savingBlockedIPs}
          />
          <Button type="button" variant="outline" onClick={onAddIP} disabled={savingBlockedIPs || blockedIPCount >= MAX_IPS}>
            {savingBlockedIPs ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Add IP'}
          </Button>
        </div>
        {newIPError === EMPTY_STRING ? null : <span className="text-xs text-destructive">{newIPError}</span>}
      </div>
    </div>
    {savingBlockedIPs ? <p className="text-xs text-muted-foreground">Updating blocked IPs...</p> : null}
    {ipActionError === EMPTY_STRING ? null : <p className="text-xs text-destructive">{ipActionError}</p>}
  </div>
);
