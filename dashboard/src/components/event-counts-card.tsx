...
const EventCountsCard: FunctionComponent<EventCountsCardProps> = ({ siteId, eventCounts, page, pageSize, onPageChange, state = 'ready' }) => {
  ...
  return (
    <Card className="transition-shadow hover:shadow-md">
      ...
      <CardContent>
        <DashboardCardState state={state} overlayLabel="Refreshing counts" skeleton={<div className="space-y-3">{SKELETON_KEYS.map((key) => <div key={key} className="flex items-center justify-between"><Skeleton className="h-4 w-28" /><Skeleton className="h-5 w-12" /></div>)}</div>}>
