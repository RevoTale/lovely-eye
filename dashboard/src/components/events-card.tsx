...
import { Badge, Card, CardContent, CardHeader, CardTitle, Skeleton, SKELETON_KEYS } from '@/components/ui';
...
const EventsCard: FunctionComponent<EventsCardProps> = ({ siteId, events, total, page, pageSize, onPageChange, state = 'ready' }) => {
  ...
  return (
    <Card className="transition-shadow hover:shadow-md">
      ...
      <CardContent>
        <DashboardCardState state={state} overlayLabel="Refreshing events" skeleton={<div className="space-y-4">{SKELETON_KEYS.slice(0, 3).map((key) => <div key={key} className="space-y-2 rounded-md border p-3"><Skeleton className="h-4 w-28" /><Skeleton className="h-3 w-20" /><div className="flex gap-2"><Skeleton className="h-5 w-16" /><Skeleton className="h-5 w-24" /></div></div>)}</div>}>
