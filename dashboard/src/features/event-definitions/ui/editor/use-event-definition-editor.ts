...
export function useEventDefinitionEditor({ onDelete, onSave }: UseEventDefinitionEditorInput) {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    const validationError = validateEventDefinition(trimmedName, normalizedFields);
    if (validationError !== '') {
...
export function validateEventDefinition(name: string, fields: EventDefinitionFieldInput[]): string {
