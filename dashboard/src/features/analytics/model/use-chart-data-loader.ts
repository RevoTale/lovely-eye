...
import {
  ...
  calculateProgress,
} from '@/features/analytics/model/chart-loader-utils';
...
export function useChartDataLoader({
  ...
  filter,
  bucket,
}: UseChartDataLoaderParams): ChartDataLoaderResult {
  ...
  const variables = useMemo(
    () => buildChartVariables(siteId, dateRange, filter, bucketValue),
  ...
  useEffect(() => {
    ...
    const dailyStats = query.data.dashboard.dailyStats as Array<
      FragmentType<typeof DailyStatsFieldsFragmentDoc>
    >;
  ...
  const loadNextBatch = useCallback(async () => {
    ...
    try {
      const result = await query.fetchMore({
        variables: { paging: { limit: BATCH_SIZE, offset: loadedData.length } },
      });
      if (result.data === undefined) return;
      const nextBatch = getFragmentData(
        DailyStatsFieldsFragmentDoc,
        result.data.dashboard.dailyStats as Array<FragmentType<typeof DailyStatsFieldsFragmentDoc>>
      );
  ...
  useEffect(() => {
    ...
    if (loadedData.length > EMPTY_COUNT && loadedData.length % BATCH_SIZE === EMPTY_COUNT)
      void loadNextBatch();
  }, [
    hasMore,
    isLoadingMore,
  ...
  const expectedCount = useMemo(
    () => calculateExpectedCount(dateRange, bucket),
    [dateRange, bucket]
  );
  const progress = useMemo(
    () => calculateProgress(expectedCount, hasMore, loadedData.length),
    [expectedCount, hasMore, loadedData.length]
  );
  const state: DashboardLoadState =
    query.loading && loadedData.length === EMPTY_COUNT
      ? 'initial'
      : query.loading
        ? 'refreshing'
        : 'ready';
