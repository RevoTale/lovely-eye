...
export const skipPollWhenHidden = (): boolean => document.visibilityState !== 'visible';
...
const pageOffset = (page: number, pageSize: number): number =>
  (page - PAGE_INDEX_OFFSET) * pageSize;
...
export const buildDashboardVariables = (
  ...
  topPagesPaging: {
    ...
    offset: pageOffset(topPagesPage, TOP_PAGES_PAGE_SIZE),
  },
  referrersPaging: {
    limit: REFERRERS_PAGE_SIZE,
    offset: pageOffset(referrersPage, REFERRERS_PAGE_SIZE),
  },
  ...
  countriesPaging: {
    ...
    offset: pageOffset(countriesPage, COUNTRIES_PAGE_SIZE),
  },
});
...
export const buildEventCountsVariables = (
  ...
  paging: {
    ...
    offset: pageOffset(eventsCountsPage, EVENTS_COUNT_PAGE_SIZE),
  },
});
