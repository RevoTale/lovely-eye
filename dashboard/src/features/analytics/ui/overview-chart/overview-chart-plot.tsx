...
import { buildOverviewAxisTicks } from '@/features/analytics/ui/overview-chart/overview-chart-axis';
import { formatOverviewAxisValue } from '@/features/analytics/ui/overview-chart/overview-chart-formatters';
import OverviewChartLegend from '@/features/analytics/ui/overview-chart/overview-chart-legend';
import {
  getOverviewSeriesIndex,
  OVERVIEW_RENDER_SERIES,
  OVERVIEW_SERIES,
  type OverviewPoint,
} from '@/features/analytics/ui/overview-chart/overview-chart-series';
import OverviewChartTooltip from '@/features/analytics/ui/overview-chart/overview-chart-tooltip';
import OverviewChartXAxisTick from '@/features/analytics/ui/overview-chart/overview-chart-x-axis-tick';
  ...
  return (
    <div className='h-[clamp(240px,44vw,320px)] w-full'>
      <AreaChart
        ...
        responsive
        style={chartStyle}
        margin={{ top: 12, right: 6, bottom: 0, left: -18 }}
      >
        <defs>
          {OVERVIEW_SERIES.map((series) => (
            <linearGradient
              key={series.key}
              id={`overview-${series.key}`}
              x1='0'
              y1='0'
              x2='0'
              y2='1'
        ...
        />
        <YAxis
          tickFormatter={(value) => formatOverviewAxisValue(Number(value))}
          tickLine={false}
          axisLine={false}
          width={44}
          className='text-xs'
