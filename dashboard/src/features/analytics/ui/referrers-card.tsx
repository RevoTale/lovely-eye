...
import {
  ...
  getReferrerIcon,
  getSafeReferrerHref,
} from '@/features/analytics/model/referrer-utils';
...
import type { ReferrerStatsFieldsFragment } from '@/shared/api/generated/graphql';
...
const ReferrersCard: FunctionComponent<ReferrersCardProps> = ({
  referrers,
  totalCount,
  totalVisitors,
  siteId,
  ...
  const maxVisitors =
    referrers.length > EMPTY_COUNT
      ? (referrers[0]?.visitors ?? FALLBACK_MAX_VISITORS)
      : FALLBACK_MAX_VISITORS;
  ...
  return (
    ...
    >
      <div className='space-y-4'>
        {referrers.length > EMPTY_COUNT ? (
          referrers.map((ref) => {
            const safeHref = getSafeReferrerHref(ref.referrer);
            const percentage =
