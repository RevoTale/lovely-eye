...
import {
  ...
  EventPropertyFieldsFragmentDoc,
...
const EventsCard: FunctionComponent<EventsCardProps> = ({
  siteId,
  events,
  total,
  page,
  ...
  return (
    <Card className='transition-shadow hover:shadow-md'>
      ...
      <CardContent>
        <DashboardCardState
          state={state}
          overlayLabel='Refreshing events'
          skeleton={
                    ...
                    <div className='flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between'>
                      <div className='min-w-0'>
                        {definitionName !== '' ? (
                          ...
                          <span className='block truncate text-sm font-medium'>{eventPath}</span>
                        )}
                        <FilterLink
                          siteId={siteId}
                          filterKey='eventPath'
                      ...
                      <div className='flex w-full flex-wrap items-center justify-between gap-2 sm:w-auto sm:shrink-0 sm:flex-nowrap'>
                        <Badge variant='outline'>
                          {definitionName !== '' ? 'Event' : 'Pageview'}
                        </Badge>
                    ...
                    {event.properties.length > EMPTY_COUNT ? (
                      <div className='mt-2 flex flex-wrap gap-2'>
                        {event.properties.map((property) => {
                          const propertyData = getFragmentData(
                            EventPropertyFieldsFragmentDoc,
                            property
                          );
                          return (
                            <Badge
                              key={`${event.id}-${propertyData.key}`}
                              variant='outline'
                              className='max-w-full'
                            >
                              <span className='break-all'>
                                {propertyData.key}: {propertyData.value}
                              </span>
                            </Badge>
                          );
                        })}
