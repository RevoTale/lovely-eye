...
import DateTimePicker from '@/features/analytics/ui/datetime-picker/date-time-picker';
...
export const TimeRangeCard = ({
  ...
  const [lastSyncKey, setLastSyncKey] = useState(
    `${fromDate}-${fromTime}-${toDate}-${toTime}-${preset}`
  );
  ...
  return (
    <Card>
      ...
      <CardContent className='space-y-4'>
        ...
        <div
          className={`overflow-hidden transition-all duration-200 ${isCustom ? 'max-h-[420px] opacity-100' : 'max-h-0 opacity-0'}`}
        >
        ...
        {submitAttempted && !draftHasValidInputs ? (
          <p className='text-xs text-destructive'>
            Select both start and end dates before applying.
          </p>
        ) : null}
