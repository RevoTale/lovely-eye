...
const BoardCard: FunctionComponent<BoardCardProps> = ({
  ...
  return (
    <Card className='transition-shadow hover:shadow-md'>
      <CardHeader>
        <CardTitle
          className={`flex items-center ${headerRight !== undefined ? 'justify-between' : 'gap-2'}`}
        >
      ...
      <CardContent>
        ...
        >
          ...
          {pagination !== undefined ? (
            <div
              className={`mt-4 flex min-h-9 ${paginationAlign === 'center' ? 'justify-center' : 'justify-start'}`}
            >
              {showPagination ? (
                <PaginationControls
