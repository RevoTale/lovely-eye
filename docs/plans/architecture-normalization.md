...
- Status: Smooth dashboard lifecycle implemented and verified; maintainer review ready
...
| C-015 | Smooth authenticated dashboard transitions | Answered | Auth bootstrap, router data coordination, Apollo lifecycle, browser coverage |
...
| D-033 | Use one neutral boot surface while a cold client-side load verifies the session. After login and throughout authenticated navigation, never cascade full-page skeleton states: keep the current stable surface until route code and critical data are ready, commit the destination atomically, retain displayed data during refresh and polling, and limit pending feedback to the affected control or a subtle non-blocking indicator. Do not render private data before server authorization. | Accepted UX invariant | 2026-08-21 | Confirmed after source-backed review of React transitions, TanStack Router loaders/preloading, Apollo cache lifecycle, and browser layout stability. Refines D-025 by replacing accepted first-load skeleton sequences with one coordinated cold-load boundary. |
| D-034 | Cross-fade the successful login or registration surface into the fully resolved authenticated destination. Let TanStack Router own the native View Transition at the final loader redirect, use the browser's unsupported-API fallback, keep the transition short, and effectively disable its duration under `prefers-reduced-motion`. Do not add an animation dependency or use React's canary-only `ViewTransition` component. | Accepted UX invariant | 2026-08-21 | Explicitly requested after D-033 verification. The final redirect owns the transition because the intermediate `/` route only selects the destination after loading accessible sites. |
| D-035 | URL-owned pagination inside analytics cards must preserve the current viewport only while the card start remains inside it. It must never reset the document to the page top; when the card start is outside the viewport, reposition exactly to the card start. | Accepted UX invariant | 2026-08-27 | Explicit user requirement; enforced centrally by pagination navigation and protected by `admin-state.spec.ts`. |

## Smooth dashboard follow-up

Status: Implemented and verified under D-033. No new dependency, tracker contract, database migration, or runtime-base-path change was required.

- [x] Replace the current Playwright skeleton expectations with an atomic login/navigation contract that fails before implementation.
- [x] Separate cold auth bootstrap from authenticated revalidation so the dashboard shell is not repeatedly unmounted.
- [x] Remove the redundant post-login session refetch while preserving cross-identity Apollo cache isolation.
- [x] Coordinate initial site selection and critical route data through TanStack Router and Apollo before committing the destination.
- [x] Retain displayable data for warm navigation, polling, filters, and refresh; reserve full placeholders for a cold no-data boundary only.
- [x] Verify direct reload, login, site switching, history, errors, reduced motion, mobile/wide layouts, CLS, GraphQL fan-out, all runtime base paths, and auth isolation.
- [x] Record before/after browser and release-matrix evidence, then restore the final maintainer-review status.
...
| 2026-08-21 | Auth destination cross-fade | Added a native TanStack Router view transition to successful login and registration. The final site-selection redirect owns the transition so route loaders finish before the old form is replaced; a 180 ms root cross-fade preserves continuity, while reduced-motion shortens it to 0.01 ms. | A Playwright RED run observed zero transitions before the change; the focused GREEN run observes exactly one transition after login. Frontend lint, boundaries, strict typechecks, 20 unit tests, all 12 primary browser flows, `/`, `/lovely-eye`, `/tools/lovely-eye`, and same-origin auth isolation pass. No dependency, API, data, or base-path contract changed. |
| 2026-08-27 | Analytics card pagination scroll stability | Disabled TanStack Router's document scroll reset for the shared URL-owned analytics pagination path, covering Recent Events, Event Counts, Top Pages, Referrers, Devices, Operating Systems, and Countries. | A Playwright RED run measured a 1,466 px jump to the document top; the focused GREEN run preserved the exact scroll offset. Full repository checks, 14 primary browser flows, `/`, `/lovely-eye`, `/tools/lovely-eye`, auth isolation, and a seeded local browser check pass. |
...
| 2026-08-21 | C-015: smooth authenticated dashboard transitions | Allow one neutral cold-start surface while the server verifies the session. After login and during authenticated navigation, keep a stable surface until critical data is ready, avoid full-page skeleton cascades, retain data during background work, and reveal the destination atomically. | Added D-033 and opened the smooth-dashboard follow-up. No new dependency is required. |
| 2026-08-21 | Auth destination cross-fade | Fade the login or registration surface out while the fully prepared authenticated destination appears, without reintroducing skeleton cascades. | Added D-034. TanStack Router and the native browser API satisfy the requirement without another UI package. |
